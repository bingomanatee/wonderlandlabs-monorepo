  ...
  describe("Tree", () => {
    ...
    describe("query", () => {
      it("should follow a record", () => {
        ...
        tree.addCollection({
          ...
          records: [{ date: FIRST_DATE, cost: 200 }],
        });
        ...
        collection.query({ identity: FIRST_DATE }).subscribe(([leaf]) => {
      ...
      it("should follow all records", () => {
        ...
        tree.query({ collection: "costs" }).subscribe((records) => {
          history.push(
            records.map((leaf) => [leaf.$identity, leaf.$value.cost]),
          );
        });
      ...
      it("should join related records", () => {
        ...
        const addressFetchedJSON = addressFetched.map((record) =>
          record.toJSON(),
        );
    ...
    describe("SINGLE collections", () => {
      it("should create a single record without a defined ID", () => {
        ...
        expect(tree.collection("bob").fetch({})).toEqual([
          { $tree: tree, $collection: "bob", $identity: SINGLE, $joins: {} },
        ]);
      });
    });
