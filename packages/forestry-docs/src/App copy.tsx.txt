...
import { loadPlaceData, placeCollection } from './CovidForestState';
...
function App() {
  ...
  useEffect(() => {
    const sub3 = placeCollection.tree.subject.pipe(debounceTime(500)).subscribe((pm) => {
    ...
    <div className={style.container}>
      {
        <Stage
          options={{
            backgroundColor: new PIXI.Color({ r: 0, g: 64, b: 180 }).toNumber(),
            resizeTo: window,
          }}
          width={width}
          height={height}
        >
          <NationMask width={width} height={height} />
          {places.map((p) => (
