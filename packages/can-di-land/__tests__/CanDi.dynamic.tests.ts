  ...
  describe("dynamic", () => {
    describe("value", () => {
      ...
      it.skip("throws when value dependencies are specified", () => {
        const can = new CanDI();
        expect(() =>
          can.add("codependent", 3000, { type: "value", deps: ["something"] }),
        ).toThrow();
      });
    });
    ...
    describe("comp", () => {
      it(
        "should return a value without deps",
        subject([], (can) => {
    ...
    it(
      "should delay execution of a comp with deps",
      subject([], (can) => {
        can.add("a+b", (a: number, b: number) => a + b, {
          type: "comp",
          deps: ["a", "b"],
        });
    ...
    it.skip("errors on infinite loops", async () => {
      ...
      const can = new CanDI();
      can.events.subscribe((event) =>
        console.log("loop event:", event.type, event.value),
      );
      can.add("a-loop", 100);
      can.add("b-loop", (a: number, c: number) => a + c, {
        type: "comp",
        deps: ["a-loop", "c-loop"],
      });
      try {
        ...
        await expect(() => {
          can.add("c-loop", (a: number, b: number) => a + b, {
            type: "comp",
            deps: ["a-loop", "b-loop"],
          });
        }).toThrow();
