  ...
  describe('constructor', () => {
    ...
    describe('type: value', () => {
      ...
      describe('async value', () => {
        it('is not present until it is resolved', async () => {
          ...
          return subject(
            [
              {
                key: 'asyncComputeOnce',
                value: pending,
                config: { type: 'value', async: true },
              },
            ],
      ...
      describe('async final value', () => {
        it(
          'is not present until it is resolved', // identical to the above $test - but async this time
          ((pending) =>
            subject(
              [
                {
                  key: 'asyncFinalComputeOnce',
                  value: pending,
    ...
    describe('type: func', () => {
       ...
       */
      it(
        'accepts func type without config',
        subject([{ key: 'bar', value: () => 2, type: 'func' }], entry__exists('bar', 'func'))
      );
      it(
        'accepts func type with config',
        subject(
      ...
      it(
        'produces a function that takes props and returns a value',
        subject(
          [
            {
              key: 'summer',
      ...
      it(
        'produces a function includes arguments and a value',
        subject(
          [
            {
              key: 'summer',
      ...
      describe('async', () => {
        ...
        it(
          'produces a function that takes props and returns an async value',
          subject(
            [
              {
                key: 'summerAsync',
    ...
    describe('type: comp', () => {
       ...
       */
      it(
        'accepts comp type without config',
        subject([{ key: 'bar', value: () => 2, type: 'comp' }], entry__exists('bar', 'comp'))
      );
      it(
        'accepts comp type with config',
        subject(
    ...
    describe('async', () => {
      ...
      it('includes arguments', async () => {
        const can = new CanDI([
          {
            key: 'asyncCompArg',
            value: (a: number, b: number) => delay(a + b, 100),
            config: {
              type: 'comp',
              args: [1, 3],
              async: true,
