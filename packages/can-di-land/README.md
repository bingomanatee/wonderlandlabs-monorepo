...
However, it is not a _requirement_ that entries are keyed with strings.
...
The _value_ of a registered item is returned by `myCan.get(mame)` -- a synchronous function.
