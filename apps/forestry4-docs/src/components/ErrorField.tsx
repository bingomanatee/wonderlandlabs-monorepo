...
const ErrorField: React.FC<ErrorFieldProps> = ({ field, branch, fieldType }) => {
    ...
    return (
      <>
        <Text fontSize="xs" color="green.600">
          ✓ Length: {quality?.length} characters {quality?.hasMinLength && quality?.hasMaxLength ? '(good)' : '(needs 3-20)'}
        </Text>
        ...
        <Text fontSize="xs" color="blue.600">
          Format: {chars?.hasLetters ? 'Letters' : ''} {chars?.hasNumbers ? '+ Numbers' : ''} {chars?.isAlphanumeric ? '(clean)' : '(special chars)'}
        </Text>
      </>
    );
  };
  ...
  const renderEmailInfo = () => {
    ...
    return (
      <>
        <Text fontSize="xs" color="blue.600">
          Provider: {domain?.isGmail ? 'Gmail' : domain?.isOutlook ? 'Outlook' : domain?.isCorporate ? 'Corporate' : 'Other'} ({domain?.domain})
        </Text>
  ...
  const renderAgeInfo = () => {
    ...
    return (
      <>
        ...
        <Text fontSize="xs" color="blue.600">
          Status: {quality?.isComplete ? 'Complete' : 'Incomplete'} • {quality?.isLegal ? 'Legal' : 'Restricted'}
        </Text>
      </>
    );
  };
