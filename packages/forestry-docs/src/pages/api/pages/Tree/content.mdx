...
* `params`: defined by TreeParams; at minimum it is `{initial: [value]}`, the value of the state at start.
  See [/api/TreeParam] for an exhaustive list of properties.
...
### `update((currentValue: <ValueType>, seed) => nextValue, seed?: any)`
...
const broadcast = tree.subscribe((value) => console.log('counter = ', value));
// counter = 0
...
adds a listener to the value of the tree. Will broadcast immedaitely when added with the
current state value.
