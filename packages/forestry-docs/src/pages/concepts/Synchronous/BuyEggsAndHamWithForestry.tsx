...
import type {
  ForestIF,
  TreeIF,
} from '@wonderlandlabs/forestry/build/src/types';
...
function makeItem(
  ...
  return {
    initialBalance,
    id: `${Math.random()}-${Date.now()}`.replace(
      '0.',
      '',
    ),
    item: '1 ' + type,
...
export function BuyEggsAndHamWithForestry() {
  ...
  const receiptsState = useRef(
    makeReceiptsState(forest.current),
  );
  const [receipts, setReceipts] = useState<
    Item[]
  >([]);

  const eggsState = useRef(
    makeNumberState(
      'eggs',
      startingEggs,
      forest.current,
  ...
  const [eggs, setEggs] = useState(
    eggsState.current?.value,
  );

  const hamState = useRef(
    makeNumberState(
      'ham',
      startingHam,
      forest.current,
  ...
  useEffect(() => {
    const subs = [
      ...
      receiptsState.current.subscribe(
        setReceipts,
      ),
    ];
    return () => {
  ...
  useEffect(() => {
    const initialBalance =
      balanceState.current.value;
    if (
      balanceState.current.value >= eggCost &&
      eggsState.current.value > 0
    ) {
      balanceState.current.update(
        (b) => b - eggCost,
      );
      ...
      receiptsState.current.update((r) => [
        ...r,
        newItem,
      ]);
    }
  ...
  useEffect(() => {
    const initialBalance =
      balanceState.current.value;
    if (
      balanceState.current.value >= hamCost &&
      hamState.current.value > 0
    ) {
      balanceState.current.update(
        (b) => b - hamCost,
      );
